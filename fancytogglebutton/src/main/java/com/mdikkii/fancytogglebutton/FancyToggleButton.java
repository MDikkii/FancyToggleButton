package com.mdikkii.fancytogglebutton;
...
import com.mdikkii.fancytogglebutton.ColorChangeTypes.ColorChangeType;
import com.mdikkii.fancytogglebutton.EaseTypes.EaseType;
import com.mdikkii.fancytogglebutton.JellyTypes.Jelly;
import com.mdikkii.fancytogglebutton.JellyTypes.JellyStyle;
...
public class FancyToggleButton extends CompoundButton {
    ...
    private static final float DEFAULT_THUMB_RADIUS_DP = 40;

    private static final float DEFAULT_BACKGROUND_MEASURE_VALUE = 4f;
    private static final float DEFAULT_BACKGROUND_RADIUS_DP = 100;

    private static final float MIN_THUMB_RADIUS_DP = 15;
    private static final float MIN_BACKGROUND_MEASURE_VALUE = 1.8f;
    private static final float MIN_BACKGROUND_RADIUS_DP = 40;

    private static final int DEFAULT_DURATION = 1000;

    private static final float DEFAULT_TOUCH_MOVE_RATIO_VALUE = 1.2f;
    ...
    public FancyToggleButton(Context context) {
        super(context);
        init(null);
    }

    public FancyToggleButton(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }

    public FancyToggleButton(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(AttributeSet attrs) {
        ...
        TypedArray ta = attrs == null ?
                null : getContext().obtainStyledAttributes(attrs, R.styleable.FancyToggleButton);
        if (ta != null) {
            mLeftBackgroundColor = ta.getColor(R.styleable.FancyToggleButton_jtbLeftBackgroundColor, mLeftBackgroundColor);
            mRightBackgroundColor = ta.getColor(R.styleable.FancyToggleButton_jtbRightBackgroundColor, mRightBackgroundColor);
            mLeftThumbColor = ta.getColor(R.styleable.FancyToggleButton_jtbLeftThumbColor, mLeftThumbColor);
            mRightThumbColor = ta.getColor(R.styleable.FancyToggleButton_jtbRightThumbColor, mRightThumbColor);
            mLeftTextColor = ta.getColor(R.styleable.FancyToggleButton_jtbLeftTextColor, mLeftTextColor);
            mRightTextColor = ta.getColor(R.styleable.FancyToggleButton_jtbRightTextColor, mRightTextColor);

            String leftTypefaceString = ta.getString(R.styleable.FancyToggleButton_jtbLeftTextTypeface);
            try {
            ...
            String rightTypefaceString = ta.getString(R.styleable.FancyToggleButton_jtbRightTextTypeface);
            try {
            ...
            mLeftTextSize = ta.getDimensionPixelSize(R.styleable.FancyToggleButton_jtbLeftTextSize, DEFAULT_LEFT_TEXT_SIZE);
            mLeftTextPaint.setTextSize(mLeftTextSize);
            mRightTextSize = ta.getDimensionPixelSize(R.styleable.FancyToggleButton_jtbRightTextSize, DEFAULT_RIGHT_TEXT_SIZE);
            mRightTextPaint.setTextSize(mRightTextSize);

            mLeftText = ta.getString(R.styleable.FancyToggleButton_jtbLeftText);
            mRightText = ta.getString(R.styleable.FancyToggleButton_jtbRightText);

            mTextMarginLeft = ta.getDimension(R.styleable.FancyToggleButton_jtbTextMarginLeft, mTextMarginLeft);
            mTextMarginRight = ta.getDimension(R.styleable.FancyToggleButton_jtbTextMarginRight, mTextMarginRight);
            mTextMarginTop = ta.getDimension(R.styleable.FancyToggleButton_jtbTextMarginTop, mTextMarginTop);
            mTextMarginBottom = ta.getDimension(R.styleable.FancyToggleButton_jtbTextMarginBottom, mTextMarginBottom);
            mTextMarginCenter = ta.getDimension(R.styleable.FancyToggleButton_jtbTextMarginCenter, mTextMarginCenter);

            mThumbRadius = ta.getDimension(R.styleable.FancyToggleButton_jtbThumbRadius, mThumbRadius);

            mBackgroundMeasureRatio = ta.getFloat(R.styleable.FancyToggleButton_jtbBackgroundMeasureRatioValue, mBackgroundMeasureRatio);
            mBackgroundRadius = ta.getDimension(R.styleable.FancyToggleButton_jtbBackgroundRadius, DEFAULT_BACKGROUND_RADIUS_DP * density);

            mDuration = ta.getInteger(R.styleable.FancyToggleButton_jtbDuration, DEFAULT_DURATION);

            mTouchMoveRatioValue = ta.getFloat(R.styleable.FancyToggleButton_jtbTouchMoveRatioValue, DEFAULT_TOUCH_MOVE_RATIO_VALUE);
            mBezierControlValue = ta.getFloat(R.styleable.FancyToggleButton_jtbBezierControlValue, mBezierControlValue);
            mStretchDistanceRatioValue = ta.getFloat(R.styleable.FancyToggleButton_jtbStretchDistanceRatioValue, mStretchDistanceRatioValue);
            mBezierScaleRatioValue = ta.getFloat(R.styleable.FancyToggleButton_jtbBezierScaleRatioValue, DEFAULT_BEZIER_SCALE_RATIO_VALUE);

            mMoveToSameStateCallListener = ta.getBoolean(R.styleable.FancyToggleButton_jtbMoveToSameStateCallListener, DEFAULT_MOVE_TO_SAME_STATE_CALL_LISTENER);
            mDraggable = ta.getBoolean(R.styleable.FancyToggleButton_jtbDraggable, DEFAULT_DRAGGABLE);

            int colorChangeTypeInteger = ta.getInteger(R.styleable.FancyToggleButton_jtbColorChangeType, -1);
            ...
            int jellyInteger = ta.getInteger(R.styleable.FancyToggleButton_jtbJelly, -1);
            if (jellyInteger != -1) mJelly = Jelly.values()[jellyInteger];
            else mJelly = DEFAULT_JELLY;

            int easeTypeInteger = ta.getInteger(R.styleable.FancyToggleButton_jtbEaseType, -1);
    ...
    private int measureWidth(int widthMeasureSpec) {
        ...
        int minWidth = (int) (mThumbRadius * 3);
    ...
    private int measureHeight(int heightMeasureSpec) {
        ...
        int minHeight = (int) (mBackgroundRadius * 2) + getPaddingTop() + getPaddingBottom();
...
//    }
    ...
    private void setup() {
        ...
        mThumbRectF.set(
                mThumbRadius * 1.5f + mThumbRadius * 1.5f,
                getMeasuredHeight() / 2 - mThumbRadius,
                mThumbRadius * 1.5f - mThumbRadius * 1.5f,
                getMeasuredHeight() / 2 + mThumbRadius);
        ...
        float thumbTop = getMeasuredHeight() / 2 - mThumbRadius;
...
//        mThumbPath.cubicTo(mThumbP4.bottom.x,mThumbP4.bottom.y,mThumbP1.left.x,mThumbP1.left.y,mThumbP1.x,mThumbP1.y);
        if (mState == State.RIGHT_TO_LEFT || mState == State.LEFT_TO_RIGHT) {
            mThumbRectF.left += abs(mBackgroundRectF.left - mBackgroundRectF.right) *  (mState == State.RIGHT_TO_LEFT || mState == State.LEFT ? mProcess * -1 : mProcess);
            mThumbRectF.right += abs(mBackgroundRectF.left - mBackgroundRectF.right) *  (mState == State.RIGHT_TO_LEFT || mState == State.LEFT ? mProcess * -1 : mProcess);
            Log.e("mProcess", mProcess + " ");
        ...
        canvas.drawRoundRect(mThumbRectF, mThumbRadius, mThumbRadius, mPaint);
    ...
    private void setAnimator(float target, final boolean callListener) {
        ...
        mProcessAnimator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                if (mProcess == 0) FancyToggleButton.super.setChecked(false);
                if (mProcess == 1) FancyToggleButton.super.setChecked(true);
    ...
    public interface OnStateChangeListener {
        void onStateChange(float process, State state, FancyToggleButton jtb);
    }
